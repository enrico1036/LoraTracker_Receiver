...
void setup()
{
    ...
    radioNode.init(LORA_CS, LORA_RST, LORA_IRQ, NULL);
    radioNode.reset();
...
*/
    if (nextSerialTaskTs < millis()) {
        if (currentBeaconIndex >= 0) {
            ...
            Serial.print("ID=");  Serial.println(beacon->getId());

            if(millis() - beacon->getLastContactMillis() > 5000)
            {
                beacon->setId(0);
            }
        }
